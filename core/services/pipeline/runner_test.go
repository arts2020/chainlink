...
func TestRunner(t *testing.T) {
	...
	t.Run("test pipeline spec error is created", func(t *testing.T) {
		...
		// We expect context cancellation errors to show up as pipeline_spec_errors.
		err = service[0].Start()
		require.NoError(t, err)
		err = service[0].Close()
		require.NoError(t, err)
		...
		require.NoError(t, err)
		require.Len(t, se, 2)
		assert.Regexp(t, ".*database timed out.*", se[0].Description)
		assert.Regexp(t, ".*context canceled.*", se[1].Description)
